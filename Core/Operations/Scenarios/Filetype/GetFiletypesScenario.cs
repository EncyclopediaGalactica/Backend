...
public class GetFiletypesScenario(DbContextOptions<DocumentDomainDbContext> dbContextOptions)
{
    ...
    private Either<ErrorResult, List<FiletypeResult>> MapToFiletypeResultList(
        List<Filetype> filetypes, Guid correlationId) => filetypes.MapToFiletypeResultList();

    private Either<ErrorResult, List<Filetype>> FetchFromDatabase(Guid              correlationId,

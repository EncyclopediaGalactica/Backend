    ...
    DbContextOptions<DocumentDomainDbContext> dbContextOptions)
{
    ...
    private Either<ErrorResult, RelationResult> CreateResult() => new RelationResult();

    private Either<ErrorResult, RelationInput> DeleteFromDatabase(RelationInput     input,
                                                                  CancellationToken cancellationToken)
    {
        using DocumentDomainDbContext ctx = new(dbContextOptions);
        try
        {
            Relation target = ctx.Relations.First(t => t.Id == input.Id);

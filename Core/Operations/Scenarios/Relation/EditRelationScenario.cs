...
)
{
    ...
    private Either<ErrorResult, Relation> UpdateDatabase(Relation entity)
    {
        using DocumentDomainDbContext ctx = new(dbContextOptions);
        try
        {
            Relation target = ctx.Relations.First(i => i.Id == entity.Id);
    ...
    private Either<ErrorResult, Relation> MapToRelationEntity(RelationInput input) =>
        mapper.MapRelationInputToRelation(input);

...
)
{
    ...
    private Either<ErrorResult, RelationResult> MapToRelationResult(Relation entity) =>
        Either<ErrorResult, RelationResult>.Right(mapper.MapRelationToRelationResult(entity));

    private Either<ErrorResult, Relation> SaveToDatabase(
        Relation          relation,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            EntityEntry<Relation> res = ctx.Relations.Add(relation);
            ctx.SaveChanges();
            return Either<ErrorResult, Relation>.Right(res.Entity);
        }
        catch (Exception e)
        {
    ...
    private Either<ErrorResult, Relation> MapToRelation(RelationInput relationInput) =>
        Either<ErrorResult, Relation>.Right(mapper.MapRelationInputToRelation(relationInput));
    ...
    private Either<ErrorResult, RelationInput> InputValidation(RelationInput input)
    {
        ...
        if (!validationResult.IsValid)
        {
            ...
            validationResult.Errors.ForEach(
                error => builder.Append($"Error at {error.PropertyName} with error message: {error.ErrorMessage}."));

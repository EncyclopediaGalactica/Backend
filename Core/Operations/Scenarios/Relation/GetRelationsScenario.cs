...
)
{
    ...
    private Either<ErrorResult, List<RelationResult>> MapToRelationResult(List<Relation> entities) =>
        relationMapper.MapRelationsToRelationResults(entities);

    private Either<ErrorResult, List<Relation>> FetchFromDatabase(
        CancellationToken cancellationToken)
    {
        using DocumentDomainDbContext ctx = new(dbContextOptions);
        try
        {
            List<Relation> result = ctx.Relations.ToList();
            return result;
        }
        catch (Exception e)
        {
            ErrorResult errorDetails = new(_correlationId, e.Message);
            return errorDetails;
        }
    }
}
